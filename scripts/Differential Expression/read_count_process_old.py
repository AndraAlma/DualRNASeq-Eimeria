...
# python scripts/read_count_process_old.py results/htseq/reverse/Day0/ results/processed_reads/Day0
...
while i < len(read_count_filenames):
    ...
    if re.search("_r", read_count_filenames[i], flags=re.IGNORECASE):
        # Make sure only read count files are read in
        l_pos = re.search("_r", read_count_filenames[i], flags=re.IGNORECASE).start()
        curr_name = read_count_filenames[i][0:l_pos]
        ...
        if curr_name != old_name:
            ...
            if i+1 == len(read_count_filenames) or not re.search(curr_name, read_count_filenames[i+1]):
                ...
                eimeria_table = sum_table.iloc[24389:32214]
                chicken_table = sum_table.iloc[np.r_[0:24389, 32214:32933], :]
